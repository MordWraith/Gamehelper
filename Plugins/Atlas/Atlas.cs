namespace Atlas
{
    ...
    public sealed class Atlas : PCore<AtlasSettings>
    {
        ...
        // Expedition map-modifier catalog (json/expeditionmods.json, generated by
        // gen_expeditionmods.py from the Mods.dat Domain=6 / GenerationType=23 pool). Two views of
        // the same table: by the id a node carries, and by the stable internal stat name the
        // settings file stores. Stat ids drift every patch — the NAME is the durable key, which is
        // why route entries are saved as "stat:<internal name>" and only this json is regenerated.
        private static readonly Dictionary<uint, ExpeditionModEntry> ExpeditionModsById = new();
        private static readonly Dictionary<string, ExpeditionModEntry> ExpeditionModsByStat =
            new(StringComparer.OrdinalIgnoreCase);

        private sealed class ExpeditionModEntry
        {
            public string Stat { get; set; }   // internal stat name, e.g. "map_expedition_explosives_+%"
            public string Name { get; set; }   // short English label for the settings row
            public uint Id;                    // Stats.dat row + 1 — what the node's mod vector holds
        }
        ...
        private struct NodeData
        {
            ...
            public uint[] ContentTokens;    // stat ids of the node's mod vector (@ element+0x350); see re-findings §2.10
            public (uint Stat, int Value)[] ModStats;  // the same vector decoded: rolled map modifiers (GetNodeModStats)
        ...
        {
            {
                ...
                for (int gi = 0; gi < Settings.ContentGroups.Count; gi++)
                {
                    ...
                    // Map Targets stays expanded while the user has no groups of their own; once they
                    // add one it collapses by default but stays freely toggleable. The Expedition-mods
                    // preset is never force-opened — its rows would bury the rest of the settings.
                    if (grp.Locked && string.Equals(grp.Key, BuiltInGroupKey, StringComparison.Ordinal)
                        && !Settings.ContentGroups.Exists(g => !g.Locked))
                    ...
                    for (int ci = 0; ci < grp.Contents.Count; ci++)
                    {
                        ...
                        if (entry.Match is { Length: > 0 } m
                            && m.StartsWith(StatMatchPrefix, StringComparison.OrdinalIgnoreCase))
                        {
                            // Map-modifier entry: show the internal stat it matches, plus the caveat
                            // that a map only exposes its modifiers once the client knows them.
                            ImGuiHelper.ToolTip(m[StatMatchPrefix.Length..] + "\n\n"
                                + this.L("atlas.expmod_entry_hint",
                                    "Routes to the nearest map that ROLLED this Expedition modifier. "
                                    + "A map's modifiers are only known once its chunk is revealed — "
                                    + "maps still under fog carry no modifier data at all."));
                        }
                        else if (LocalizedDesc(entry.ContentName) is { Length: > 0 } d)
                        {
        ...
        private void AppendMistNodesMissedByCore(UiElement atlasUi, int atlasCount)
        {
            ...
            for (int i = 0; i < atlasCount; i++)
            {
                ...
                var modStats = GetNodeModStats(addr);
                var contentTokens = ModStatIds(modStats);
        ...
        private bool TryRefreshNodeCacheFromCore(int atlasCount)
        {
            try
            {
                ...
                foreach (var map in maps)
                {
                    ...
                    var mapInfo = GetMapInfo(internalId);
                    var modStats = DecodeCoreModStats(ToUintArray(nTokens?.GetValue(map)));
                    ...
                    newCache.Add(new NodeData
                    {
                        ...
                        ContentTokens = ModStatIds(modStats),
                        ModStats = modStats,
                        BadgeContentIds = badgeIds,
                        ContentNames = BuildContentNames(ModStatIds(modStats), badgeIds, internalId, nodeAddr),
        ...
        // Load the Expedition map-modifier catalog (json/expeditionmods.json): { "<statId>": { stat,
        // name } }. Missing/unreadable file = the "Expedition mods" group simply matches nothing
        // (its entries stay listed, so the user's colours/hops survive a bad regen).
        private void LoadExpeditionMods()
        {
            ExpeditionModsById.Clear();
            ExpeditionModsByStat.Clear();
            var path = Path.Join(DllDirectory, "json", "expeditionmods.json");
            if (!File.Exists(path))
                return;

            var entries = JsonConvert.DeserializeObject<Dictionary<string, ExpeditionModEntry>>(File.ReadAllText(path));
            if (entries is null)
                return;
            ...
            {
                var e = kv.Value;
                if (e is null || string.IsNullOrWhiteSpace(e.Stat) || !uint.TryParse(kv.Key, out var id))
                    continue;
                e.Id = id;
                if (string.IsNullOrWhiteSpace(e.Name))
                    e.Name = e.Stat;
                ExpeditionModsById[id] = e;
                ExpeditionModsByStat[e.Stat] = e;
            }
        }
        ...
        private const string BuiltInGroupKey = "maptargets";
        // Second locked group: route to maps that ROLLED a given Expedition modifier. Entries are
        // matched by "stat:<internal stat name>" against the node's own mod vector (GetNodeModStats),
        // so this only ever lights up maps whose modifier the client already knows — on the sea that
        // means a charted chunk (a fogged node carries no mod data at all).
        private const string ExpeditionModsGroupName = "Expedition mods";
        private const string ExpeditionModsGroupKey = "expeditionmods";
        // Preset palette (the user's hex values): #3357E6 / #FCFCFC / #FDD400 / #FD0000 / #FD7300.
        // Declared BEFORE the preset array: static field initializers run in declaration order, so a
        // palette defined after it would still be default(Vector4) — i.e. transparent black — when
        // the array captures it, with no compiler warning.
        private static readonly Vector4 ModBlue = new(0.2f, 0.34117647f, 0.9019608f, 1f);
        private static readonly Vector4 ModWhite = new(0.9882353f, 0.9882353f, 0.9882353f, 1f);
        private static readonly Vector4 ModYellow = new(0.99215686f, 0.83137256f, 0f, 1f);
        private static readonly Vector4 ModRed = new(0.99215686f, 0f, 0f, 1f);
        private static readonly Vector4 ModOrange = new(0.99215686f, 0.4509804f, 0f, 1f);

        // The preset, in the order it is listed in the UI. Stats come from json/expeditionmods.json
        // (Mods.dat Domain=6 / GenerationType=23 — what an atlas map can roll); a stat missing from
        // that json is still listed but matches nothing.
        //
        // This is a CURATED subset of that pool, not all of it: the quantity-ish rolls (relics,
        // artifacts, verisium, vendor currency, the four chest-marker counts, both monster-marker
        // counts) were dropped at the user's request — they roll often and aren't worth sailing to,
        // so listing them only made the group long. json/expeditionmods.json still carries them, so
        // re-adding one is a single line here.
        //
        // Colours and the on/off defaults below are the user's own tuned setup: the remnant rolls
        // (the ones worth crossing the map for) are ON in yellow/red/orange, and the explosive-
        // placement trio plus the two monster rolls are listed but OFF in blue/white — they matter
        // once you are already there, not for choosing where to sail. Only a fresh install sees
        // these; an existing group keeps whatever the user set (entries reconcile by Match key).
        private static readonly (string Stat, Vector4 Color, bool On, int Hops)[] BuiltInExpeditionMods =
        {
            ("map_expedition_explosives_+%",                     ModBlue,   false, 25),
            ("map_expedition_explosion_radius_+%",               ModBlue,   false, 25),
            ("map_expedition_maximum_placement_distance_+%",     ModBlue,   false, 25),
            ("map_expedition_twinned_elites",                    ModWhite,  false, 25),
            ("map_expedition_saga_contains_boss",                ModYellow, false, 25),
            ("map_logbook_expedition_remnants_+",                ModYellow, true,  25),
            ("map_expedition2_remnants_have_at_least_x_slots",   ModRed,    true,  25),
            ("map_logbook_has_at_least_1_expedition2_remnant_with_at_least_x_slots", ModRed, true, 25),
            ("map_logbook_has_at_least_1_expedition2_remnant_with_a_power_rune",     ModOrange, true, 25),
            ("map_expedition2_remnant_generation_has_x_lucky_rolls", ModYellow, true, 25),
        };
        ...
        // Make sure both locked built-in groups exist and their content lists match the fixed presets,
        // preserving any per-entry style the user has customised (matched by the Match key). Neither
        // group's entries can be added or removed in the UI — only toggled/recoloured/re-hopped, and
        // the group's line thickness adjusted.
        private void EnsureBuiltInContentGroups()
        {
            if (Settings?.ContentGroups == null)
                return;

            // Expedition mods seed the group master OFF: its routes are a deliberate "I am farming
            // this today" choice, unlike Map Targets (citadels/mothersoul), which is useful from the
            // first launch. The off-screen markers ride on the same master switch.
            EnsureLockedGroup(ExpeditionModsGroupKey, ExpeditionModsGroupName, 1.5f,
                System.Array.ConvertAll(BuiltInExpeditionMods, e => ("stat:" + e.Stat, e.Color, e.On, e.Hops)),
                seedDrawPaths: false);
            EnsureLockedGroup(BuiltInGroupKey, BuiltInGroupName, 1.5f, BuiltInTargets, seedDrawPaths: true);

            // Locked groups first and in preset order (Expedition mods above Map Targets); user groups
            // keep their relative order below them. Sorting here rather than on insert keeps the order
            // stable for configs written before the second preset existed.
            int Rank(ContentGroupSettings g) => g.Key switch
            {
                ExpeditionModsGroupKey => 0,
                BuiltInGroupKey => 1,
                _ => 2,
            };
            var ordered = new List<ContentGroupSettings>(Settings.ContentGroups);
            ordered.Sort((a, b) => Rank(a).CompareTo(Rank(b)));
            Settings.ContentGroups.Clear();
            Settings.ContentGroups.AddRange(ordered);
        }

        // Reconcile one locked group against its preset. Entries are matched by their Match key, so a
        // user's colour/hops/toggle survive a preset reorder, a rename, or a json regen; entries that
        // left the preset are dropped and new ones appear with the preset default.
        private void EnsureLockedGroup(string key, string name, float thickness,
            (string Match, Vector4 Color, bool On, int Hops)[] preset, bool seedDrawPaths)
        {
            var grp = Settings.ContentGroups.Find(g => string.Equals(g.Key, key, StringComparison.OrdinalIgnoreCase));
            if (grp == null && string.Equals(key, BuiltInGroupKey, StringComparison.Ordinal))
            {
                // Config written before groups carried a key: the single locked group there is the
                // map-targets preset. Adopt it instead of creating a duplicate.
                grp = Settings.ContentGroups.Find(g => g.Locked && string.IsNullOrEmpty(g.Key));
            }

            bool freshGroup = grp == null;
            if (freshGroup)
            {
                grp = new ContentGroupSettings { Name = name, LineThickness = thickness };
            ...
            var reconciled = new List<ContentRouteEntry>(preset.Length);
            bool anyMatched = false;
            foreach (var (match, color, on, hops) in preset)
            {
                // Fallback label = the matcher value (after "id:" / "name:" / "stat:"); the UI resolves
                // the localized display name from it via ContentEntryDisplayName.
                ...
                if (existing != null)
                {
                    existing.ContentName = label;  // fallback label; UI shows the resolved name
            ...
            // Fresh group, or migration from an older preset (no entry matched the new keys): apply the
            // preset's seed for the group master. An up-to-date group keeps the user's choice.
            if (freshGroup || !anyMatched)
                grp.DrawPaths = seedDrawPaths;
        }

        // Matcher prefix for a rolled map modifier: "stat:<internal stat name>" (see BuiltInExpeditionMods).
        private const string StatMatchPrefix = "stat:";

        // Does this node carry the modifier named by `stat`? The node stores modifiers by stat id, and
        // ids drift every patch, so the stable internal NAME is resolved through the catalog first —
        // an unknown name (json missing or regenerated without it) matches nothing rather than
        // matching whatever id happens to sit there now.
        private static bool MatchModStat(string stat, in NodeData nd)
        {
            if (string.IsNullOrEmpty(stat)
                || nd.ModStats is not { Length: > 0 }
                || !ExpeditionModsByStat.TryGetValue(stat, out var entry))
                return false;

            foreach (var (id, _) in nd.ModStats)
            {
                if (id == entry.Id)
                    return true;
            }
        ...
        private string ContentEntryDisplayName(ContentRouteEntry e)
        {
            ...
            if (!string.IsNullOrEmpty(e.Match) && e.Match.StartsWith(StatMatchPrefix, StringComparison.OrdinalIgnoreCase))
            {
                // Map-modifier entry: the plugin's own UI dictionary first (translatable), then the
                // catalog's English label, then the raw stat name so a missing json still reads.
                var stat = e.Match[StatMatchPrefix.Length..];
                var fallback = ExpeditionModsByStat.TryGetValue(stat, out var mod) ? mod.Name : stat;
                return this.L("atlas.expmod." + stat, fallback);
            }
            ...
            List<(Vector2 Target, int Hops, string Label, Vector4 Color)> markers, float uiScale, float range)
        {
            ...
            float margin = 60f * uiScale;
            var min = new Vector2(margin, margin);
            var max = new Vector2(MathF.Max(margin + 1f, display.X - margin),
                                  MathF.Max(margin + 1f, display.Y - margin));
            ...
            var placed = new List<RectangleF>(markers.Count);
            var takenEdges = new HashSet<(string, int)>();

            foreach (var m in markers)
            {
                var edge = GetLineRectangleIntersection(m.Target, center, min, max);

                // Which border the point landed on — 0 left, 1 right, 2 top, 3 bottom. Used both to
                // de-duplicate per mod and to decide which way overlapping pills stack.
                float dl = MathF.Abs(edge.X - min.X), dr = MathF.Abs(edge.X - max.X);
                float dt = MathF.Abs(edge.Y - min.Y), db = MathF.Abs(edge.Y - max.Y);
                float best = MathF.Min(MathF.Min(dl, dr), MathF.Min(dt, db));
                int side = best == dl ? 0 : best == dr ? 1 : best == dt ? 2 : 3;
                bool vertical = side is 0 or 1;   // pills on the left/right borders stack downwards

                if (!takenEdges.Add((m.Label, side)))
                    continue;

                var label = m.Hops >= 0
                    ? "[" + m.Hops.ToString(CultureInfo.InvariantCulture) + "] " + m.Label
                    : m.Label;
                float w = MathF.Max(pillH, ImGui.CalcTextSize(label).X + 8f * uiScale);

                // Keep the whole pill on screen, then nudge it along its border until it stops
                // overlapping a pill already placed there.
                float cx = Math.Clamp(edge.X, w * 0.5f + 2f, MathF.Max(w * 0.5f + 2f, display.X - w * 0.5f - 2f));
                float top = Math.Clamp(edge.Y - pillH * 0.5f, 2f, MathF.Max(2f, display.Y - pillH - 2f));
                for (int attempt = 0; attempt < 12; attempt++)
                {
                    var rect = new RectangleF(cx - w * 0.5f, top, w, pillH);
                    if (!placed.Exists(r => r.IntersectsWith(rect)))
                        break;
                    if (vertical)
                        top += step;
                    else
        ...
        // ── Node map modifiers ───────────────────────────────────────────────────────────────────
        // The +0x350 vector is NOT a pool of content ids: it is std::vector<{u32 StatId; u32 Value}>
        // (stride 8) — the node's rolled MAP MODIFIERS, the same layout as the active zone's mod
        // vector (AreaInstance+0x150) and with the same off-by-one, `StatId = Stats.dat row + 1`.
        // A content marker is just a pair whose Value is 1 ("Area contains Expedition"); a pair with
        // Value != 1 is a numeric modifier and Value is the number the game's tooltip prints.
        // Verified live 0.5.5b: node 1214 ExpeditionLogBook_Tropical → (13473, 44) =
        // map_expedition_explosives_+% = "+44% increased number of Expedition explosives".
        // See obsidian poe2/Atlas.md §"Спец-модификаторы карт на нодах".
        //
        // Reading them as a flat u32 array (what this plugin used to do) makes every second word —
        // the VALUE — look like a content id, and throws the magnitude away.
        public static (uint Stat, int Value)[] GetNodeModStats(IntPtr nodeAddr)
            => DecodeModPairs(GetContentTokens(nodeAddr));

        private static readonly (uint Stat, int Value)[] NoModStats = System.Array.Empty<(uint, int)>();

        // Raw vector words (self-read path) → {StatId, Value} pairs. An odd word count means a torn
        // read, so the whole list is dropped rather than shifted by one.
        public static (uint Stat, int Value)[] DecodeModPairs(uint[] raw)
        {
            if (raw is not { Length: > 1 } || (raw.Length & 1) != 0)
                return NoModStats;

            var mods = new (uint, int)[raw.Length / 2];
            for (int i = 0; i < mods.Length; i++)
                mods[i] = (raw[i * 2] & 0xFFFFu, (int)raw[i * 2 + 1]);
            return mods;
        }

        // GameHelper's core reader hands out the same pairs already re-packed into its legacy
        // "token" shape, `(Value * 64) << 16 | StatId` (ImportantUiElements.ReadAtlasContentTokens) —
        // one word per modifier instead of two. Undo that when the node came from the core path.
        public static (uint Stat, int Value)[] DecodeCoreModStats(uint[] tokens)
        {
            if (tokens is not { Length: > 0 })
                return NoModStats;

            // Packed entries always carry a non-zero high word (Value >= 1 → Value*64 >= 64). An
            // all-zero high word means this core hands out the RAW pairs instead, so read them as
            // pairs rather than silently reporting every modifier as value 0.
            bool packed = false;
            ...
            if (!packed)
                return DecodeModPairs(tokens);

            var mods = new (uint, int)[tokens.Length];
            for (int i = 0; i < tokens.Length; i++)
                mods[i] = (tokens[i] & 0xFFFFu, (int)((tokens[i] >> 16) / 64u));
            return mods;
        }

        // Stat ids alone, for the content-name resolution path (it only ever looks at the id).
        private static uint[] ModStatIds((uint Stat, int Value)[] mods)
        {
            if (mods is not { Length: > 0 })
                return System.Array.Empty<uint>();

            var ids = new uint[mods.Length];
            for (int i = 0; i < mods.Length; i++)
                ids[i] = mods[i].Stat;
            return ids;
        }
        ...
        private bool MatchContentRoute(in NodeData nd, out ContentRouteEntry match, out ContentGroupSettings matchGroup)
        {
            ...
            foreach (var grp in Settings.ContentGroups)
            {
                ...
                foreach (var entry in grp.Contents)
                {
                    ...
                    // Built-in entries match by map id/classification or by a rolled map modifier;
                    // user entries by node content.
                    if (!string.IsNullOrEmpty(entry.Match))
                    {
                        if (entry.Match.StartsWith(StatMatchPrefix, StringComparison.OrdinalIgnoreCase))
                        {
                            if (MatchModStat(entry.Match[StatMatchPrefix.Length..], in nd))
                            {
                                match = entry;
                                matchGroup = grp;
                                return true;
